...
let string_of_checks checks =
	...
	String.concat "" (List.map untitled checks)
...
let translate narration =
	...
	let add_process_actions = function
		| Exchange (a_agent, b_agent, message) ->
			...
			let message_representation = Knowledge.exchange a_knowledge b_knowledge message narration.equations in
	...
	equational_theory ^ agent_projections ^ global_process
